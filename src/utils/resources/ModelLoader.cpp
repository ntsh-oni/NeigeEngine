							...
							if (primitive->material->has_pbr_metallic_roughness) {
								...
								if (baseColorTexture != NULL) {
									...
									if (textures.find(baseColorImage->uri) == textures.end()) {
										...
										ImageTools::createImageView(&image.imageView, image.image, 0, 1, image.mipmapLevels, VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
								...
								if (metallicRoughnessTexture != NULL) {
									...
									if (textures.find(metallicRoughnessImage->uri) == textures.end()) {
										...
										ImageTools::createImageView(&image.imageView, image.image, 0, 1, image.mipmapLevels, VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
										ImageTools::createImageSampler(&image.imageSampler, image.mipmapLevels, VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_REPEAT, VK_BORDER_COLOR_FLOAT_OPAQUE_BLACK);
