...
void Model::draw(CommandBuffer* commandBuffer, bool bindTextures) {
	...
	for (Mesh& mesh : meshes) {
		for (size_t i = 0; i < mesh.primitives.size(); i++) {
			if (bindTextures) {
				mesh.descriptorSets.at(i).bind(commandBuffer, 1);
			}
...
void Model::createDescriptorSets(GraphicsPipeline* graphicsPipeline) {
	for (Mesh& mesh : meshes) {
		...
		for (size_t i = 0; i < mesh.primitives.size(); i++) {
			mesh.descriptorSets.at(i).init(graphicsPipeline, 1);
			...
			diffuseWriteDescriptorSet.dstSet = mesh.descriptorSets[i].descriptorSet;
			...
			normalWriteDescriptorSet.dstSet = mesh.descriptorSets[i].descriptorSet;
			...
			metallicRoughnessWriteDescriptorSet.dstSet = mesh.descriptorSets[i].descriptorSet;
			...
			occlusionWriteDescriptorSet.dstSet = mesh.descriptorSets[i].descriptorSet;
			...
			mesh.descriptorSets[i].update(writesDescriptorSet);
		}
	}
}
