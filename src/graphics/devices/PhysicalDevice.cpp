...
bool PhysicalDevice::isSuitable(const Surface* surface) {
	...
	if (queueFamilyIndices.isComplete()) {
		if (extensionSupport()) {
			...
			return !deviceSwapchainSupport.formats.empty() && !deviceSwapchainSupport.presentModes.empty() && features.fillModeNonSolid && features.samplerAnisotropy && features.sampleRateShading && features.independentBlend;
		}
	}
	return false;
}
